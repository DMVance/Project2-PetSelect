...
def find_location(search_location):
    ...
    # ---print output for testing---
    location_query = db.dogs.find(location_query_dict)

    location_list = []
    for dog in location_query:
        location_list.append(dog)
    location_df = pd.DataFrame(location_list).drop(columns=['_id'])

    print("""
    ----------------------------------------
    Dog location search (Sample of first 10 results)
    ----------------------------------------
    """)
    print(location_df.head(10))

    return location_query_dict


# FIND TYPE (Stray, Owner Surrender, Public Assist, Abandoned)
def find_type(search_type):
    type_query_dict = {
        "intake_type": {
            "$regex": search_type, 
            "$options": "i"
    ...
    # ---print output for testing---
    type_query = db.dogs.find(type_query_dict)

    type_list = []
    for dog in type_query:
        type_list.append(dog)
    type_df = pd.DataFrame(type_list).drop(columns=['_id'])

    print("""
    ----------------------------------------
    Dog type search (Sample of first 10 results)
    ----------------------------------------
    """)
    print(type_df.head(10))

    return type_query_dict
...
def combined_queries(search_breed, youngest_yrs, youngest_mos, oldest_yrs, oldest_mos, search_sex, search_color, search_location, search_type, search_injured):
    list_of_query_dicts = [
...
def user_input():
    ...
    print("""
        Find a dog of a specific location!
        """)
    search_location = input("Desired location: ")

    print("""
        Find a dog of a specific type!
        """)
    search_type = input("Desired stray or owner surrender or public assist or abandoned: ")
